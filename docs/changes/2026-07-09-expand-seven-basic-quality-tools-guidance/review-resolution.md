...
| CR-M2-001 | `docs/changes/2026-07-09-expand-seven-basic-quality-tools-guidance/reviews/code-review-m2-r1.md` | closed | accepted | Revised the Scatter good-versus-weak prompt record to remove the causal-arrow request; replaced `media/scatter-diagram/good-vs-weak-scatter.png` with a weak-side example showing unlabeled axes, missing units, hidden outlier, clutter, unpaired-looking points, and unsupported annotation clutter without a causal or trend arrow; reran M2 prompt/media/link proof, manual image review, and `git diff --check`. |
