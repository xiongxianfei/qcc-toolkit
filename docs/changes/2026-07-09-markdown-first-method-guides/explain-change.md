...
| M2 Pareto method kit: `method-kits/pareto-chart.md`, `media/pareto-chart/` | Added a single Pareto method guide containing method use, manual chart creation, interpretation, review checklist, evidence note, worked example, and reviewed good/bad example notes; media stores prompts and reviewed teaching visuals. | Pareto Chart is the first complete proof that the Markdown-first kit model can teach method use, manual chart creation, interpretation, and review without a method-folder file explosion. | Spec R5-R10, R16-R22, R26-R27; plan M2. | Pareto kit tests in `tests/test_markdown_first_method_guidance.py`; M2 code review. |
